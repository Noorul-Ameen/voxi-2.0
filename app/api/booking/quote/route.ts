...
export async function POST(request: Request) {
  ...
  try {
    ...
    const tickets = (ticketResponse?.Tickets || []).filter((ticket: any) => Number.isFinite(Number(ticket.PriceInCents))).sort((a: any, b: any) => Number(a.PriceInCents) - Number(b.PriceInCents));
    if (!tickets.length) throw new VoxApiError("No priced ticket type is available.", { status: 409, code: "NO_TICKET_TYPES" });
    const items = seats.map((seatId: string) => ({ seatId, amount: Number(tickets[0].PriceInCents) / 100, ticketTypeCode: tickets[0].TicketTypeCode }));
